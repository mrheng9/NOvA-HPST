...
from hpst.models.pointnet import PointNetInterface
...
class PointNetTrainer(NeutrinoBase):
    def __init__(self, options: Options, train_perc=None):
        ...
        """
        super(PointNetTrainer, self).__init__(options, train_perc=train_perc)

        self.num_objects = 10
        self.num_classes = self.training_dataset.num_classes
        self.network = PointNetInterface(options,
                            self.training_dataset.num_features + self.training_dataset.coord_dim,
