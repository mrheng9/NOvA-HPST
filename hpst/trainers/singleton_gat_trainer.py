...
#from hpst.models.singleton_point_set_transformer import PointSetTransformerInterface
from hpst.models.singleton_gat import GATInterface
...
@torch.jit.script
def collate_sparse(data: List[Tuple[int, List[Tensor]]]):
    ...
    return (hits_index, batches1, features1, coordinates1, targets1, object_targets1)
...
class GATTrainer(NeutrinoBase):
    ...
    def forward(self, features1: Tensor, coords1: Tensor, batch1: Tensor) -> Tensor:
        # Normalize the high level layers
        features1 = features1.clone()
        features1 -= self.mean
        features1 /= self.std

        outputs1 = self.network((coords1, features1, batch1))
        
        predictions1, object_predictions1 = outputs1[:, :self.num_classes], outputs1[:, self.num_classes:]

        return predictions1, object_predictions1
    ...
    def bipartite_loss(self, logits, targets, batches):
        ...
        #print("a", targets.amin(), targets.amax())
        #print(object_preds.shape, targets.shape)
        #print(targets)
        #print(batches)
    ...
    def training_step(self, batch, batch_idx):
        ...
        (_, batches1, features1, coordinates1, targets1, object_targets1) = batch

        predictions1, object_predictions1 = self.forward(features1, coordinates1, batches1)
